...
class PasswordResetter
{
    ...
    public function initiatePasswordResetProcess($loginOrEmail, $newPassword)
    {
        ...
        $randomToken = base64_encode(random_bytes(64));
        $this->savePasswordResetInfo($login, $newPassword, $randomToken);

        // ... send email with confirmation link
        try {
            $this->sendEmailConfirmationLink($user, $randomToken);
        } catch (Exception $ex) {
    ...
    public function checkValidConfirmPasswordToken($login, $resetToken)
    {
        ...
        if ($resetInfo === false
            ...
            || empty($resetInfo['randomTokenHash'])
            || empty($resetInfo['timestamp'])
            || !$this->isTokenValid($resetToken, $resetInfo['randomTokenHash'])
        ) {
     ...
     * @param string $randomTokenHash The hashed random token.
     * @return bool true if valid, false otherwise.
     */
    public function isTokenValid($token, $randomTokenHash)
    {
        if ($token['timestamp'] + $this->getDefaultExpiryDuration() < time()) {
            return false;
        }
        return $this->passwordHelper->verify($token, $randomTokenHash);
    }
    ...
    /**
     * Returns an expiration duration of a password reset token. By default it will be one
     * day (24 hrs) from now.
     *
     ...
     * @return int
     */
    protected function getDefaultExpiryDuration()
    {
        return 24 * 60 * 60; /* +24 hrs */
    }
     ...
     * @param string $randomToken A secure randomly generated token.
     */
    private function sendEmailConfirmationLink($user, $randomToken)
    {
        $login = $user['login'];
        $email = $user['email'];
        ...
        $url = Url::getCurrentUrlWithoutQueryString()
            ...
            . "&resetToken=" . urlencode($randomToken);
     ...
     */
    private function savePasswordResetInfo($login, $newPassword, $randomToken)
    {
        ...
        if ($existingResetInfo) {
            ...
            if (isset($existingResetInfo['timestamp']) && $existingResetInfo['timestamp'] > time()-3600 && isset($existingResetInfo['randomTokenHash'])) {
        ...
        $optionData = [
            ...
            'randomTokenHash' => $this->passwordHelper->hash($randomToken),
            'timestamp' => $time,
            'requests' => $count+1
        ];
